...
methodology) lives in the project owner's private
`SONNET/URTC-FLASHER/auditoria_historial.txt` and is not published here.
...
increments instead (e.g. `1.1.9` -> `1.2.0`), and the same carry applies
...
- Ecosystem-wide build-versioning policy: `FLASHER_VERSION` normalized from
  the old two-part `"1.1"` to the three-part `"1.1.0"`, and `bump_version.py`
...
Visual/text version bump (banner SVGs, splash text) from v1.0 to v1.1,
plus a full pass fixing every leftover "v1.0"/"1.0" reference across the
...
after the user reported still seeing "v1.0" post-migration.
